...
<xsl:stylesheet
					...
					<xsl:for-each select="PEXR2002/IDOC">
						<S_ST>
							...
							<S_CUR>
								...
								<D_100>
									<xsl:value-of
										select="E1IDKU5[MOAQUAL=002]/CUXWAERZ" />
								</D_100>
							</S_CUR>
							<S_REF>
								<D_128>CK</D_128>
								...
								<xsl:choose>
									<xsl:when test="E1EDKA1[PARVW = 'BE']/ILNNR != ''">
										<D_352>B1</D_352>
									</xsl:when>
									<xsl:otherwise>
										<D_352>A1</D_352>
							...
							<S_REF>
								...
								<xsl:choose>
									<xsl:when
										test="boolean(E1EDKA1[PARVW = 'BE']/KNREF = 'C')">
										<!-- If customer Should default HQ1 -->
										<D_127>00HQ1</D_127>
									</xsl:when>
									<xsl:when
										test="boolean(E1IDPU1/E1EDP02[QUALF = '071']/BELNR)">
										<D_127>
									...
									<L_RMR>
										<S_RMR>
											...
											<D_782>
												<xsl:choose>
													<xsl:when
														test="contains(E1IDPU5[MOAQUAL=006]/MOABETR,'-')">
														<xsl:value-of
											...
											<D_782_2>
												<xsl:choose>
													<xsl:when
														test="contains(E1IDPU5[MOAQUAL=004]/MOABETR,'-')">
														<xsl:value-of
											...
											<D_782_3>
												<xsl:choose>
													<xsl:when
														test="contains(E1IDPU5[MOAQUAL=003]/MOABETR,'-')">
														<xsl:value-of
	...
	<xsl:template name="prepend-pad">
		<!-- recursive template to right justify and prepend the value with whatever 
			padChar is passed -->
